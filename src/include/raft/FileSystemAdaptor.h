...
#ifndef  RAFT_FILE_SYSTEM_ADAPTOR_H
#define  RAFT_FILE_SYSTEM_ADAPTOR_H
...
public:
    ...
    virtual size_t write(const string& data, off_t offset) = 0;
    ...
    virtual size_t read(string& portal, off_t offset, size_t size) = 0;
...
public:
    ...
    virtual FileAdaptor* open(const std::string& path, int oflag, 
                              const horsedb::TarsStructBase* file_meta,
                              int* e) = 0;
...
protected:
    PosixDirReader(const std::string& path) : _path(path) {}

private:
    struct dirent* _dir_reader;
    std::string _path;
};
...
public:
    ...
    virtual size_t write(const string& data,  off_t offset);
    virtual size_t read( string& portal,  off_t offset, size_t size);
    virtual size_t size();
    virtual bool sync();
    virtual bool close();
